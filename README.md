...
- Recognizes incremental-save versioning - Blender's native Save Incremental (`name_001.blend`, `name_002.blend`, ...) and BB_Kitsu-Pipeline's style (`name_v001.blend`, `name_v002.blend`, ...) both work, since grouping only needs "same name before the trailing number." When another matching version exists next to the current file, the log shows each version's own total plus the sum across all of them - so you can answer "how long did this shot take in total," not just "how long did this particular version take."
...
- **Also Save CSV** - write a companion `<blendfile>_time.csv` alongside the text log, one row per session (file, date, start, end, duration, user). Unlike the text log's by-day/by-user summaries, this is deliberately flat and unaggregated - drop it straight into Excel/Sheets to build your own totals, pivots, or charts. Off by default.
- **Show Version Totals** - show each incremental-save version's total and the sum across all of them, when other versions exist next to the current file. On by default. Independent of the other detail toggles above, so it can be shown or hidden on its own regardless of what else is - e.g. keep per-user totals visible for accountability while trimming daily/session detail.
